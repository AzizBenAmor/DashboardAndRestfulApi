...
@include('dashboard.admin.layout.navprofession')
       <div class="content">
                        ...
                        <table class="table">
                          ...
                          <tbody>
                            <tr class="alert" role="alert">
                              ...
                              <td>
                                <div class="email">
                                 <a style="color: rgb(0, 0, 0)" href="{{ route('admin.SpecialiteIndex',$p->id) }}" ><span>{{ $p->nom }} </span></a>
                              ...
                              <td class="quantity">
                                <button class="btn btn-phoenix-success me-1 mb-1" style="width: 110px; border: 1px solid rgb(24, 23, 23); " type="button" data-bs-toggle="modal" data-bs-target="#verticallyCentered{{ $p->id }}">Consulter</button>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-content">
    <div class="modal-header">
      <h5 class="modal-title" id="exampleModalLabel">Ajouter Profession</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times fs--1"></span></button>
    </div>
     
    <div class="modal-body">
  
    
    <form action="{{ route('admin.ProfessionStore',$idsecteur) }}" method="post" enctype="multipart/form-data">
      @csrf
    ...
    <div class="mb-3">
      ...
      <input name="nom" class="form-control" id="exampleFormControlInput1" type="text" placeholder="Nom de Profession " >
      @error('nom')
 ...
 <div class="modal fade" id="verticallyCentered{{ $s->id }}" tabindex="-1" aria-labelledby="verticallyCenteredModalLabel" aria-hidden="true">
     <div class="modal-dialog modal-dialog-scrollable">
         <div class="modal-content">
           <div class="modal-header">
             <h5 class="modal-title" id="scrollingLongModalLabel2">Profession</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times fs--1"></span></button>
           </div>
           <div class="modal-body">
             <div class="row">
                 ...
                 <div class="col-9">
                   {{ $s->nom }}
                   <div class="row">
                    @foreach ($specialite as $p)
                    @if ($s->id==$p->profession_id)
                    {{ $p->nom }}<br>  
                    @endif
 ...
 <div class="modal fade" id="exampleModal{{ $s->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog">
     <div class="modal-content">
       <div class="modal-header">
         <h5 class="modal-title" id="exampleModalLabel">Modifier Profession</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times fs--1"></span></button>
       </div>
       <div class="modal-body">
         <form action="{{ route('admin.ProfessionUpdate',$s->id) }}" method="POSt" enctype="multipart/form-data">
             @method('PUT')
        @csrf
        <div class="mb-3">
         <label class="form-label" for="exampleFormControlInput1">Profession </label>
         <input name="nomU" class="form-control" id="exampleFormControlInput1" type="text" placeholder="Nom de profession" required value="{{ $s->nom }}">
         @error('nomU')
         <div class="alert alert-soft-danger" role="alert">{{ $message }}</div>
         @enderror
       </div>
